...
void start_compress()
{
	...
	rewind(input);//volta pro comeco do file
...
void decompress(){
	...
	fseek(output, -1, SEEK_END);//ir para o byte do trash
	unsigned char aux_bit = 0;
	aux_bit = fgetc(output);

	long long int bytes_arq = ftell(output);//calc size of the file
	...
	rewind(output);
	c = fgetc(output); // catch the first byte..
	...
	fscanf(output,"%c", &tree_size); // catch the second byte...
	...
	h_tree *tree = NULL;
	tree = montar_arvore(output,tree,tree_size,0); // a partir da pre-ordem
	...
	while(fscanf(output,"%c",&c) != EOF) // lendo o arquivo que devemos descompactar...
	{
		...
		for(j = 7; j >= 0; j--)
		{
		   ...
		   {  
				fputc(tree->c, novoarquivo); // colocando no descompactado
			 	tree = raiz;
		   }
		}
		//--------------------------
